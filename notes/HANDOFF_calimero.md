...
2. **継続騎乗ボーナス** — `fetch_horse.py` で取得済みの `recent[i].jockey` を使い、当日騎手と前走騎手が同じなら +1 を `calimero_bonus()` に追加
3. **昇級ボーナス** — `recent[i].race_class` と shutuba の `race_name` から昇級判定 → +1 (カリメロ分析: 昇級含む◎ 複勝18.4%/単回収429%)
4. **Cal重みのチューニング** — `weights["cal"]` を 0.5, 1.0, 1.5, 2.0 で振り、過去結果に対するROI最大化点を探す
